...
namespace Connect4Graphic
{
    public partial class FormGame : Form
    {
        ...
        public FormGame()
        {
            // $G$ SFN-009 (-2) The name is not as required
            Text = "Connect 4 in A Raw!";
        ...
        {
            // $G$ DSN-999 (-8) You shouldn't have reviewed the whole matrix for each selection
            for (int i = m_Matrix.Board.GetLength(0) - 1; i >= 0; i--)
            {
                for (int j = m_Matrix.Board.GetLength(1) - 1; j >= 0; j--)
                {
                    ...
                    // $G$ SFN-999 (-3) You shouldn't check that for evey cell in the matrix but for any row/column
        ...
        private bool checkIfWinOrDraw(PlayerInfo i_PlayFirst)
        {
            ...
            {
                drawGame();
                // $G$ SFN-009 (-2) The message isn't correct
                if (MessageBox.Show(
                    "It's A Draw!!" +
                    Environment.NewLine +
                    "Play another round?",
